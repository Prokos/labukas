...
await page
  ...
  .click();
await page
  .getByRole("heading", { name: "Something delicious", exact: true, level: 1 })
  .waitFor();
...
await page.getByText("NOTICE THE PATTERN").waitFor();
await page.getByRole("button", { name: "Let’s try it" }).click();
...
if (
  (await c1.locator(".chapter-toggle").getAttribute("aria-expanded")) !== "true"
)
...
await c1
  ...
  .click();
await page
  .getByText("Finish the chapter’s learning sessions before taking its check.")
  .waitFor();
  ...
  (key) =>
    localStorage.setItem(
      ...
      JSON.stringify({
        version: 1,
        events: [
          { id: "old-pass", type: "complete", lesson: "c2l1", at: Date.now() },
        ],
      }),
    ),
  STORAGE_KEY,
...
await mobile.getByText("self-reviewed, not graded", { exact: true }).waitFor();
