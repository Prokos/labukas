...
The app also checks an uncached build-version endpoint on open/resume and every minute, independently of service-worker lifecycle notifications. Reloads remain deferred during lessons and settings. Lessons stay in the browser’s normal display mode without requesting or exiting fullscreen. Answer inputs autofocus on touch devices as well as desktop. The compact progress label identifies the chapter, module, and lesson within that module.
